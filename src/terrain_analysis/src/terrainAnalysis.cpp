...
// cloud clearing callback function，留给用户使用
...
int main(int argc, char **argv)
{
  ...
  while (status) {
    ros::spinOnce();
    // laserCloudHandler 回调之后
    if (newlaserCloud) {
      newlaserCloud = false;

      // terrain voxel roll over，按默认值，这两个变量开始都是0
